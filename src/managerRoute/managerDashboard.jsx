...
export default function ManagerDashboard(){
   ...
   const bookingToday = bookings.filter(booking=>booking.date.slice(0,10) === today)
    ...
    const cancelBooking = async(id)=>{
        ...
        try{
            await axios.patch(`http://localhost:3000/api/user/cancelbooking/${selectedBooking}`,{
                headers : {
                    'Authorization' : `Bearer ${token}`
                }
            })
    ...
    return(
        <> <button onClick={()=>{navigate(-1)}} className=" text-white bg-green-500 rounded-md p-1 m-2 ">Back</button>
           ...
           <h1 className="text-4xl">Today's Bookings</h1>
            ...
            <div className="flex flex-col p-3 w-full gap-3">
            {
                    bookingToday && bookingToday.length > 0 ?( bookingToday.map((item,index)=>{
                       ...
                       return (
                        <div key={item._id} className="flex flex-col gap-4 border border-gray-300 p-4 rounded-lg shadow-md bg-white">
                ...
                }
                {
                    confrim && (
                        ...
                        <div className="flex flex-col gap-3 bg-white w-96 p-5 rounded-md"> 
                         ...
                         <h1 className="text-xl p-1">Are you sure to cancel ?</h1>
            ...
            {seen &&( <>
                                   ...
                                   <div className="flex flex-col gap-3 bg-white w-96 p-5 rounded-md"> 
                                    ...
                                    <select onChange={(e)=>{setTimeslot(JSON.parse(e.target.value))}} className="p-1" name="timslot" id="">
